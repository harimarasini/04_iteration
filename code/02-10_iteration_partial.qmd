...
nematode %>%
  ...
  ggplot(aes(x = value)) +
  geom_density() +
  facet_wrap(~name, scales = "free")   # facet_wrap splits one plot into multiple smaller plots, one for each level of a variable.
...
nematode_meancv <- data.frame(variable = rep(NA, 26),
                              mean = rep(NA, 26),
                              cv = rep(NA, 26)
                              ) # we have 26 rows, so creating empty 26 rows with NA
...
nematode_mods_sel <- nematode_mods %>% 
  unnest(r2) %>% 
  group_by(resp_var) %>% 
  #filter(r2 == max(r2))    [either slice_max or filter is okay to pull out best r2 or model]
  slice_max(r2)
